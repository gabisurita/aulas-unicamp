...
public class ContaCor {
	...
	protected float obterSaldo(){
		return obterSaldo(senha);
	}
	...
	public boolean creditarValor(String hist, float val) {
		...
		if (val<=0){
			System.out.println("Valor de credito incorreto:"+val);
			return false;
		}
		
		if (estado == ContaCor.ENCERRADA){
	...
	public String[] historico(int senhaCliente){
	...
	public float[] lancamentos(int senhaCliente){
