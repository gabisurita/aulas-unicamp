...
public class ControladorCaixa {
	...
	public boolean transferir(int num, int numDestino, int senha, float val){
		
		ContaCor cta;
		ContaCor dest;
		
		cta=dbContas.buscarConta(num);
		dest=dbContas.buscarConta(numDestino);
		
		
		if (cta==null || dest==null)  
			return false; 

		if (cta.debitarValor("Transferencia para "+numDestino, val, senha)==false)
			return false;
		else{
			dest.creditarValor("Transferencia de "+num, val);
			return true;
		}
	}
	
	public String consultarExtrato(int num, int senha){
		...
		historico = cta.historico(senha);
		
		if(historico == null){
			return "";
		}
		
		lancamentos = cta.lancamentos(senha);
	
		String ext = "";
		
		for(int i=0; i<lancamentos.length; i++){
			if(historico[i] != null)
				ext += lancamentos[i] + "  " + historico[i]+"\n"; 
		}
		
		return ext;
	}
	
	public boolean depositar(int num, float val){
		...
		cta.creditarValor("Deposito em caixa automatico", val);
	...
	public boolean renderPoupanca(int num){
		...
		if(cta instanceof ContaPoupanca)
			poup = (ContaPoupanca)cta;
		else
			return false;
